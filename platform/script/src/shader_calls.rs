...
impl ShaderFnCompiler {
    ...
    pub(crate) fn handle_texture_builtin_exec(
        ...
        match method_id {
            ...
            id!(sample) | id!(sample_as_bgra) | id!(sample_as_bgra_repeat) | id!(sample_lod)
            | id!(sample_nearest) | id!(sample_rt) => {
                ...
                // sample_as_bgra_repeat(coord) is sample_as_bgra with a REPEAT address
                // mode instead of clamp-to-edge, so tiling world textures wrap on raw
                // UVs without the mip-LOD blowup that software fract() causes at seams.
                ...
                // These two flags fold the repeat variant into the bgra code paths.
                let is_bgra =
                    method_id == id!(sample_as_bgra) || method_id == id!(sample_as_bgra_repeat);
                let is_repeat = method_id == id!(sample_as_bgra_repeat);
                let method_name = if method_id == id!(sample_as_bgra) {
                    "sample_as_bgra"
                } else if method_id == id!(sample_as_bgra_repeat) {
                    "sample_as_bgra_repeat"
