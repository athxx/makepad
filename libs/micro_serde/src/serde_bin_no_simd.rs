...
pub trait SerBin {
    ...
    #[inline]
    fn ser_bin_slice_size_hint(slice: &[Self]) -> usize
    where
        Self: Sized,
    {
    ...
    /// Serialize a contiguous slice. Scalar POD types override this with one
    /// bulk copy on little-endian targets.
    #[inline]
    fn ser_bin_slice(slice: &[Self], out: &mut Vec<u8>)
    where
        Self: Sized,
    {
        for item in slice {
...
pub trait DeBin: Sized {
    ...
    #[inline]
    fn deserialize_bin(data: &[u8]) -> Result<Self, DeBinErr> {
        let mut offset = 0;
...
macro_rules! impl_ser_de_bin_for {
    ($ty:ident) => {
        impl SerBin for $ty {
            ...
            #[inline(always)]
            fn ser_bin_slice_size_hint(slice: &[$ty]) -> usize {
                std::mem::size_of_val(slice)
            }

            #[cfg(target_endian = "little")]
            #[inline]
            fn ser_bin_slice(slice: &[$ty], out: &mut Vec<u8>) {
...
impl SerBin for usize {
    ...
    #[inline(always)]
    fn ser_bin_slice_size_hint(slice: &[usize]) -> usize {
        sat_mul_usize(slice.len(), 8)
    }

    #[inline]
    fn ser_bin_slice(slice: &[usize], out: &mut Vec<u8>) {
...
impl SerBin for LiveId {
    ...
    #[inline(always)]
    fn ser_bin_slice_size_hint(slice: &[LiveId]) -> usize {
        sat_mul_usize(slice.len(), 8)
    }

    #[inline]
    fn ser_bin_slice(slice: &[LiveId], out: &mut Vec<u8>) {
...
impl DeBin for LiveId {
    ...
    #[inline(always)]
    fn de_bin(offset: &mut usize, data: &[u8]) -> Result<LiveId, DeBinErr> {
        Ok(LiveId(u64::de_bin(offset, data)?))
    }
    ...
    fn de_bin_slice(
        ...
        let start = *offset;
        let remaining = remaining_bytes(start, data)
            .ok_or_else(|| de_bin_error(start, WIDTH, data.len(), "Vec<LiveId> length exceeds buffer"))?;
        if count > remaining / WIDTH {
            return Err(de_bin_error(start, WIDTH, data.len(), "Vec<LiveId> length exceeds buffer"));
        }
        let old_len = out.len();
        out.reserve(count);
        let dst = unsafe { out.as_mut_ptr().add(old_len) };
        for index in 0..count {
            ...
            unsafe { dst.add(index).write(LiveId(u64::from_le_bytes(bytes))) };
        }
        unsafe { out.set_len(old_len + count) };
        *offset = start + count * WIDTH;
        Ok(())
    ...
    fn de_bin_array<const N: usize>(
        offset: &mut usize,
        data: &[u8],
    ) -> Result<[LiveId; N], DeBinErr> {
        const WIDTH: usize = 8;
        let start = *offset;
        let remaining = remaining_bytes(start, data)
            .ok_or_else(|| de_bin_error(start, WIDTH, data.len(), "LiveId"))?;
        if N > remaining / WIDTH {
            return Err(de_bin_error(start, WIDTH, data.len(), "LiveId"));
        }
        let mut out = MaybeUninit::<[LiveId; N]>::uninit();
        let dst = out.as_mut_ptr().cast::<LiveId>();
        for index in 0..N {
            let source_offset = start + index * WIDTH;
            let bytes = unsafe {
                (data.as_ptr().add(source_offset) as *const [u8; WIDTH]).read_unaligned()
...
impl SerBin for u8 {
    ...
    #[inline(always)]
    fn ser_bin_slice_size_hint(slice: &[u8]) -> usize {
        slice.len()
    }

    #[inline]
    fn ser_bin_slice(slice: &[u8], out: &mut Vec<u8>) {
...
impl SerBin for i8 {
    ...
    #[inline(always)]
    fn ser_bin_slice_size_hint(slice: &[i8]) -> usize {
        slice.len()
    }

    #[inline]
    fn ser_bin_slice(slice: &[i8], out: &mut Vec<u8>) {
...
impl SerBin for bool {
    ...
    #[inline(always)]
    fn ser_bin_slice_size_hint(slice: &[bool]) -> usize {
        slice.len()
    }

    #[inline]
    fn ser_bin_slice(slice: &[bool], out: &mut Vec<u8>) {
...
impl SerBin for String {
    ...
    #[inline]
    fn ser_bin_slice_size_hint(slice: &[String]) -> usize {
    ...
    T: SerBin,
{
    #[inline]
    fn ser_bin(&self, out: &mut Vec<u8>) {
        let payload_hint = T::ser_bin_slice_size_hint(self);
    ...
    #[inline]
    fn ser_bin_size_hint(&self) -> usize {
        sat_add_usize(8, T::ser_bin_slice_size_hint(self))
    }

    #[inline]
    fn ser_bin_slice_size_hint(slice: &[Vec<T>]) -> usize {
        slice.iter().fold(0usize, |total, value| {
            sat_add_usize(total, value.ser_bin_size_hint())
        })
    }
}
    ...
    T: SerBin,
{
    ...
    fn ser_bin_size_hint(&self) -> usize {
        match self {
            None => 1,
            Some(value) => sat_add_usize(1, value.ser_bin_size_hint()),
        }
    }

    #[inline]
    fn ser_bin_slice_size_hint(slice: &[Option<T>]) -> usize {
        slice.iter().fold(0usize, |total, value| {
            sat_add_usize(total, value.ser_bin_size_hint())
        })
    }
    ...
    E: SerBin,
{
    ...
    fn ser_bin_size_hint(&self) -> usize {
        match self {
            Ok(value) => sat_add_usize(1, value.ser_bin_size_hint()),
            Err(error) => sat_add_usize(1, error.ser_bin_size_hint()),
        }
    }

    #[inline]
    fn ser_bin_slice_size_hint(slice: &[Result<T, E>]) -> usize {
        slice.iter().fold(0usize, |total, value| {
            sat_add_usize(total, value.ser_bin_size_hint())
        })
    }
    ...
    T: SerBin,
{
    #[inline]
    fn ser_bin(&self, out: &mut Vec<u8>) {
        let hint = T::ser_bin_slice_size_hint(self);
        if hint != usize::MAX {
            out.reserve(hint);
        }
        T::ser_bin_slice(self, out);
    ...
    #[inline]
    fn ser_bin_size_hint(&self) -> usize {
        T::ser_bin_slice_size_hint(self)
    }
}
    ...
    B: SerBin,
{
    ...
    #[inline(always)]
    fn ser_bin_size_hint(&self) -> usize {
        sat_add_usize(self.0.ser_bin_size_hint(), self.1.ser_bin_size_hint())
    }
}
    ...
    C: SerBin,
{
    ...
    #[inline(always)]
    fn ser_bin_size_hint(&self) -> usize {
        sat_add_usize(sat_add_usize(self.0.ser_bin_size_hint(), self.1.ser_bin_size_hint()), self.2.ser_bin_size_hint())
    }
}
    ...
    D: SerBin,
{
    ...
    #[inline(always)]
    fn ser_bin_size_hint(&self) -> usize {
        sat_add_usize(sat_add_usize(sat_add_usize(self.0.ser_bin_size_hint(), self.1.ser_bin_size_hint()), self.2.ser_bin_size_hint()), self.3.ser_bin_size_hint())
    }
}
    ...
    E: SerBin,
{
    ...
    #[inline(always)]
    fn ser_bin_size_hint(&self) -> usize {
        sat_add_usize(sat_add_usize(sat_add_usize(sat_add_usize(self.0.ser_bin_size_hint(), self.1.ser_bin_size_hint()), self.2.ser_bin_size_hint()), self.3.ser_bin_size_hint()), self.4.ser_bin_size_hint())
    }
}
    ...
    V: SerBin,
{
    #[inline]
    fn ser_bin(&self, out: &mut Vec<u8>) {
        let estimate = self.ser_bin_size_hint();
...
impl SerBin for PathBuf {
    #[inline(always)]
    fn ser_bin(&self, out: &mut Vec<u8>) {
        self.as_os_str().ser_bin(out)
    }

    #[inline(always)]
    fn ser_bin_size_hint(&self) -> usize {
        self.as_os_str().ser_bin_size_hint()
    }
}
