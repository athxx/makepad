...
pub trait SerJson {
    ...
    fn serialize_json(&self) -> String {
        self.serialize_json_with_capacity(128)
    }

    #[inline]
    fn serialize_json_with_capacity(&self, capacity: usize) -> String {
    ...
    /// Serializes into an existing buffer and reuses its allocation.
    #[inline]
    fn serialize_json_to(&self, out: &mut String) {
        let mut s = SerJsonState {
...
impl_ser_de_json_unsigned!(usize, usize::MAX, write_u64, u64);
impl_ser_de_json_unsigned!(u64, u64::MAX, write_u64, u64);
impl_ser_de_json_unsigned!(u128, u128::MAX, write_u128, u128);
impl_ser_de_json_unsigned!(u32, u32::MAX, write_u64, u64);
impl_ser_de_json_unsigned!(u16, u16::MAX, write_u64, u64);
impl_ser_de_json_unsigned!(u8, u8::MAX, write_u64, u64);
impl_ser_de_json_signed!(i64, i64::MIN, i64::MAX, write_i64, i64);
impl_ser_de_json_signed!(i128, i128::MIN, i128::MAX, write_i128, i128);
impl_ser_de_json_signed!(i32, i32::MIN, i32::MAX, write_i64, i64);
impl_ser_de_json_signed!(i16, i16::MIN, i16::MAX, write_i64, i64);
impl_ser_de_json_signed!(i8, i8::MIN, i8::MAX, write_i64, i64);
    ...
    T: SerJson,
{
    fn ser_json(&self, d: usize, s: &mut SerJsonState) {
        ...
        if let Some(first) = values.next() {
            s.indent(d + 1);
            first.ser_json(d + 1, s);
            for value in values {
                s.out.push(',');
                s.indent(d + 1);
                value.ser_json(d + 1, s);
            }
        }
        s.out.push(']');
    }
...
impl<'a> SerJson for JsonValueRef<'a> {
    fn ser_json(&self, d: usize, s: &mut SerJsonState) {
        match self {
            JsonValueRef::String(value) => value.ser_json(d, s),
            JsonValueRef::U64(value) => value.ser_json(d, s),
            JsonValueRef::U128(value) => value.ser_json(d, s),
            JsonValueRef::I64(value) => value.ser_json(d, s),
            JsonValueRef::I128(value) => value.ser_json(d, s),
            JsonValueRef::F64(value) => value.ser_json(d, s),
            JsonValueRef::Bool(value) => value.ser_json(d, s),
            JsonValueRef::BareIdent(value) => value.ser_json(d, s),
            JsonValueRef::Null => s.out.push_str("null"),
            JsonValueRef::Undefined => s.out.push_str("undefined"),
            JsonValueRef::Object(value) => value.ser_json(d, s),
            JsonValueRef::Array(value) => value.ser_json(d, s),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn serialize_none_as_json_null() {
    ...
    fn integer_ranges_are_checked_before_casting() {
        assert_eq!(u8::deserialize_json("255").unwrap(), 255);
        assert!(u8::deserialize_json("256").is_err());
        assert_eq!(i8::deserialize_json("-128").unwrap(), -128);
        assert!(i8::deserialize_json("-129").is_err());
        assert!(i32::deserialize_json("2147483648").is_err());
    }
    ...
    fn integer_fast_writers_match_standard_formatting() {
        for value in [
            ...
            assert_eq!(value.serialize_json(), value.to_string());
        }
    ...
    fn integer_parser_ignores_many_leading_zeroes_in_compat_mode() {
        ...
        assert_eq!(u128::deserialize_json(&text).unwrap(), 1);
    }

    #[test]
    fn long_float_uses_reusable_fallback_buffer() {
        let text = format!("0.{}1", "0".repeat(80));
        let value = f64::deserialize_json(&text).unwrap();
    ...
    fn long_string_fast_path_roundtrips() {
        ...
        let encoded = value.serialize_json();
        let decoded = String::deserialize_json(&encoded).unwrap();
        assert_eq!(decoded, value);
    }

    #[test]
    fn unicode_surrogate_pair_is_decoded() {
        let decoded = String::deserialize_json("\"\\uD83D\\uDE80\"").unwrap();
        assert_eq!(decoded, "🚀");
    }
