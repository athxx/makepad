// micro_serde is dependency-free: it ships its own binary (SerBin/DeBin) and
// JSON (SerJson/DeJson) codecs, no serde. (The RON codec was removed, so this
// example no longer exercises SerRon/DeRon.)
