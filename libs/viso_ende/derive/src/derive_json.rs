    ...
    for attr in &field.attrs {
        if attr.name == "rename" {
            if let Some(ref args) = attr.args {
                let mut ps = TokenParser::new(args.clone());
    ...
    fields
        ...
        .map(|(index, field)| {
            ...
            JsonField {
                ...
                json_name,
                local_name: format!("__viso_json_field_{}", index),
...
#[inline]
fn add_optional_open(tb: &mut TokenBuilder, field: &JsonField<'_>, source: EnFieldSource) {
    tb.add("if let Some ( __viso_json_value ) = ");
    match source {
        EnFieldSource::Struct => {
            tb.add("& self .").ident(&field.field.name);
        }
        EnFieldSource::Binding => {
...
fn add_required_en(
    ...
    tb.add(". en_json ( d + 1 , s ) ;");
}

#[inline]
fn add_optional_en(tb: &mut TokenBuilder) {
    tb.add("__viso_json_value . en_json ( d + 1 , s ) ;");
}
...
fn add_named_en(
    ...
    if first_required == Some(0) {
        ...
        add_required_en(tb, &fields[0], source);
        ...
        for field in &fields[1..] {
            if field.optional {
                ...
                add_push_str(tb, &field.comma_key_prefix);
                add_required_en(tb, field, source);
            }
        }
    ...
    tb.add("let mut __viso_json_first = true ;");
    ...
    for (index, field) in fields[..optional_prefix_len].iter().enumerate() {
        ...
        if index == 0 {
            tb.add("__viso_json_first = false ;");
            add_push_str(tb, &field.key_prefix);
        } else {
            tb.add("if __viso_json_first { __viso_json_first = false ;");
            add_push_str(tb, &field.key_prefix);
            tb.add("} else {");
            add_push_str(tb, &field.comma_key_prefix);
            tb.add("}");
    ...
    if let Some(required_index) = first_required {
        ...
        tb.add("let _ = __viso_json_first ;");
    }

    add_push_str(tb, close);
}

fn add_tuple_struct_en(tb: &mut TokenBuilder, count: usize) {
    if count == 0 {
        add_push_str(tb, "[]");
        return;
    }
...
fn add_tuple_variant_en(
    ...
    for (index, binding) in bindings.iter().enumerate() {
        ...
        tb.ident(binding).add(". en_json ( d , s ) ;");
    }
    add_push_str(tb, "]}");
}
    ...
    for field in fields {
        ...
        if field.optional {
            tb.add("if let Some ( __viso_json_value ) = ")
                ...
                .add("{ __viso_json_value } else { None } ,");
        } else {
            tb.add("if let Some ( __viso_json_value ) = ")
                .ident(&field.local_name)
                .add("{ __viso_json_value } else { return std :: result :: Result :: Err ( s . err_nf (");
...
fn add_tuple_de_values(tb: &mut TokenBuilder, count: usize) {
    for _ in 0..count {
        tb.add("{ let __viso_json_value = DeJson :: de_json ( s , i ) ? ; s . eat_comma_block ( i ) ? ; __viso_json_value } ,");
    }
}

pub fn derive_en_json_impl(input: TokenStream) -> TokenStream {
    ...
    if ps.eat_ident("struct") {
        if let Some(name) = ps.eat_any_ident() {
            ...
            let where_clause = ps.eat_where_clause(Some("EnJson"));
            ...
            tb.add("EnJson for")
                ...
                .stream(generic)
                .stream(where_clause);
            tb.add("{ # [ inline ] fn en_json ( & self , d : usize , s : & mut EnJsonState ) {");

            if let Some(types) = types {
                add_tuple_struct_en(&mut tb, types.len());
            } else if let Some(fields) = ps.eat_all_struct_fields() {
                let fields = collect_json_fields(&fields);
                add_named_en(&mut tb, &fields, EnFieldSource::Struct, "{", "}");
            } else {
        ...
        if let Some(name) = ps.eat_any_ident() {
            ...
            tb.add("EnJson for")
                ...
                .stream(generic)
                .stream(where_clause);
            tb.add("{ # [ inline ] fn en_json ( & self , d : usize , s : & mut EnJsonState ) { match self {");
                ...
                if let Some(variant) = ps.eat_any_ident() {
                    if let Some(types) = ps.eat_all_types() {
                        let bindings: Vec<String> = (0..types.len())
                            .map(|index| format!("__viso_json_variant_{}", index))
                            .collect();
                        ...
                        tb.add(") => {");
                        add_tuple_variant_en(&mut tb, &variant, &bindings);
                        tb.add("}");
                    } else if let Some(fields) = ps.eat_all_struct_fields() {
                        let fields = collect_json_fields(&fields);
...
pub fn derive_de_json_impl(input: TokenStream) -> TokenStream {
    ...
    if ps.eat_ident("struct") {
        if let Some(name) = ps.eat_any_ident() {
            let generic = ps.eat_generic();
            let types = ps.eat_all_types();
            let where_clause = ps.eat_where_clause(Some("DeJson"));
            ...
            if let Some(types) = types {
                ...
                tb.add("let __viso_json_result = Self (");
                add_tuple_de_values(&mut tb, types.len());
                tb.add(") ;");
                tb.add("s . block_close ( i ) ? ;");
                tb.add("std :: result :: Result :: Ok ( __viso_json_result )");
            } else if let Some(fields) = ps.eat_all_struct_fields() {
                let fields = collect_json_fields(&fields);
        ...
        if let Some(name) = ps.eat_any_ident() {
            ...
            tb.add("s . string ( i ) ? ;");
            tb.add("s . colon ( i ) ? ;");
            tb.add("let __viso_json_result = match s . strbuf . as_str ( ) {");
            ...
            while !ps.eat_eot() {
                ...
                if let Some(variant) = ps.eat_any_ident() {
                    ...
                    if let Some(types) = ps.eat_all_types() {
                        tb.add("s . block_open ( i ) ? ;");
                        tb.add("let __viso_json_variant = Self ::")
                            .ident(&variant)
                            .add("(");
                        add_tuple_de_values(&mut tb, types.len());
                        tb.add(") ;");
                        tb.add("s . block_close ( i ) ? ;");
                        tb.add("__viso_json_variant");
                    } else if let Some(fields) = ps.eat_all_struct_fields() {
                        let fields = collect_json_fields(&fields);
                        add_named_de_parse(&mut tb, &fields);
                        tb.add("Self ::").ident(&variant).add("{");
