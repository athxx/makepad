#!/usr/bin/env python3
"""Submit one image-input job (default trellis-2) and print every progress-label
change with timestamps:
  mesh_probe.py <base_url> <input.png> [out.glb|out.png] [--seed N] [--model ID] [--prompt TEXT]"""
...
model = sys.argv[sys.argv.index("--model") + 1] if "--model" in sys.argv else "trellis-2"
req = {"model": model, "input_b64": base64.b64encode(open(png, "rb").read()).decode()}
if "--prompt" in sys.argv:
    req["prompt"] = sys.argv[sys.argv.index("--prompt") + 1]
if seed is not None:
    req["seed"] = seed
t0 = time.time()
...
if out and j.get("state") == "done":
    ...
    for a in arts:
        ...
        if "gltf" in ct or "glb" in ct or ct.startswith("image/"):
